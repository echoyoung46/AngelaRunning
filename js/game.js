...
game.states.start = function () {
	this.preload = function () {
		...
		this.create = function () {
			...
			this.angela.width = 150;
			this.angela.height = 150;
		...
		this.update = function () {
			this.signal.add(function (e) {

				if (e.clientX < game.width/2) {
					this.angela.y = this.angela.height;
					// this.angela.scale.x = -1;
				} else {
					this.angela.y = game.height - this.angela.height / 2;
					// this.angela.scale.x = 1;
				}

				if (e.clientY < game.height / 3 * 2 && this.angela.body.onFloor()) {
					this.angela.y = -800;
				}
		...
		this.addProps = function () { //添加道具
			...
			if (this.score >= 100) { //分数大于300后再随机道具里添加乌鸦
				...
				if (small !== null) {
					// this.angela.x = this.angela.width;
					// this.angela.y = game.height - this.angela.height / 2;
					small.reset(game.width, game.height - this.angela.height / 2);
